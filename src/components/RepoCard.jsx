...
export default function RepoCard({ repo, isPinned, isShelf, tags, togglePin, toggleShelf, onSelect }) {
    ...
    <div className={`repo-card ${isPinned ? "is-pinned" : ""} ${isShelf ? "is-shelf" : ""}`}>
        ...
        {repo.topics?.length > 0 && (
          <div className="card-topics">
            ...
            {repo.topics.length > 5 && (
              <span className="topic-more">+{repo.topics.length - 5}</span>
            )}
          </div>
        )}
      </div>
      ...
      <div className="card-footer">
        <div className="card-stats">
          <span className="stat">⭐ {formatStars(repo.stars)}</span>
          <span className="stat">🍴 {formatStars(repo.forks)}</span>
          <span className="stat">📅 {timeAgo(repo.last_commit)}</span>
        </div>
