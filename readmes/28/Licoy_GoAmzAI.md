...
| 🔐 内置行为滑动验证 | 支持内置的行为滑动验证码两种方式，可运用于登录、注册等需要验证行为的场景，完全内置，无需任何第三方API | ✅ | ✅ | ✅ |
| 🧩 动态多工具集插件系统 | 可以直接在线编辑插件，实时动态编辑运行，支持JS、Python、PHP等 | ✅ | ✅ | ✅ |
| 🛜 MCP（SSE模式）工具调用支持 | 支持MCP的SSE协议工具调用，在后台直接管理MCP工具，然后加载工具，动态开关，对话模块动态启用使用等。 | ✅ | ✅ | ✅ |
| 🛜 MCP（StreamableHTTP、Stdio模式）工具调用支持 | 支持MCP的SSE、StreamableHTTP、Stdio协议工具调用，在后台直接管理MCP工具，然后加载工具，动态开关，对话模块动态启用使用等。 | ❌ | ✅ | ✅ |
| 🏠 自定义首页 | 自定义首页内容 | ❌ | ✅ | ✅ |
| 🔐 多点登录限制 | 限制用户多设备同时登录 | ❌ | ✅ | ✅ |
